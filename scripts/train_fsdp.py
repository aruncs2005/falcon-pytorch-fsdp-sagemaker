...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path,use_fast=False,cache_dir=args.cache_dir)
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path,cache_dir=args.cache_dir,trust_remote_code=True, torch_dtype=torch.bfloat16)
    ...
    falcon_auto_wrap_policy = functools.partial(
        ...
        transformer_layer_cls={
            get_module_class_from_name(model,"DecoderLayer"),
        },
    )
    ...
    check_fn_gpt = lambda submodule: isinstance(submodule, get_module_class_from_name(model,"DecoderLayer"))
